...
describe("Weak Randomness", function () {
  ...
  beforeEach(async function () {
    ...
    const LotteryV2 = await ethers.getContractFactory("LotteryV2", deployer);
    this.lotteryV2 = await LotteryV2.deploy();
  });
  ...
  describe("Lottery", function () {
    describe.skip("With bets open", function () {
      ...
      it("Should revert if a user place more than 1 bet", async function () {
        ...
        await expect(
          this.lottery.placeBet(150, { value: ethers.utils.parseEther("10") })
        ).to.be.revertedWith("Only 1 bet per player");
      });
      it("Should revert if bet is != 10 eth", async function () {
        await expect(
          this.lottery.placeBet(150, { value: ethers.utils.parseEther("5") })
        ).to.be.revertedWith("Bet cost: 10 ether");
        await expect(
          this.lottery.placeBet(150, { value: ethers.utils.parseEther("15") })
        ).to.be.revertedWith("Bet cost: 10 ether");
      });
      it("Should revert if bet number is not > 0", async function () {
        await expect(
          this.lottery.placeBet(0, { value: ethers.utils.parseEther("10") })
    ...
    describe.skip("Attack", function () {
      ...
      it("Replicate random logic within the same block", async function () {
        await this.lotteryAttacker.attack({
          value: ethers.utils.parseEther("10"),
        });
        await this.lottery.endLottery();
    ...
    describe("Lottery with Chainlink", function () {
      it("Should generate random result", async function () {
        await this.lotteryV2.getRandomNumber();

        const randomResult = await this.lottery.randomResult;
        console.log("randomResult", randomResult);
        expect((await this.lottery.randomResult) != null);
      });
    });
  });
});
